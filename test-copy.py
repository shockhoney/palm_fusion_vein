...
from train_teacher import get_transforms
from sem_CKD_student_mobilefacenet import TinyMobileFaceNet
...
class Config:
    ...
    stage2_ckpt = os.path.join("outputs_distill", "sem-CKD_models", "student_last_semckd.pth")
...
def main():
    ...
    cnn_palm = TinyMobileFaceNet(input_channel=3, embedding_size=256).to(device)
    cnn_vein = TinyMobileFaceNet(input_channel=3, embedding_size=256).to(device)
    feat_dim = 256
    
    fusion_model = Stage2FusionStudent_BottleneckGate(
        in_dim_global=feat_dim, out_dim_final=512, final_l2norm=True
    ).to(device)
