...
class Stage2FusionSS(nn.Module):
    def __init__(self):
        super(Stage2FusionSS, self).__init__()

    def forward(self, palm_feat, vein_feat):
        # Element-wise simple continuous sum
        return palm_feat + vein_feat
from utils.datasets_txt import TxtImageDataset, PairTxtDataset
...
def train_phase2(cnn_palm, cnn_vein, config, writer, feat_dim, local_dim):
    ...
    fusion_model = Stage2FusionSS().to(config.device)
    ...
    for epoch in range(config.p2_epochs):
        ...
        if avg_val_acc > best_acc:
            ...
            torch.save({
                ...
                'classifier': classifier.state_dict()
            }, os.path.join(config.save_dir, 'stage2_SS_best.pth'))
