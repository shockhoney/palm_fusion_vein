...
class Stage2FusionStudent_BottleneckGate(nn.Module):
    def __init__(self, in_dim_global=256, out_dim_final=512,
        ...
        self.proj = nn.Linear(bottleneck, out_dim_final, bias=True)
    ...
    def forward(self, F_palm, F_vein):
        ...
        w = self.gate(cross).view(p.size(0), 2, self.d)         # [B,2,d]
        w = F.softmax(w, dim=1)
