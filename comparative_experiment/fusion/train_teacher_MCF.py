...
class Stage2FusionMCF(nn.Module):
    def __init__(self, in_dim=256, final_l2norm=True):
        super(Stage2FusionMCF, self).__init__()
        # Learnable weight \omega
        self.omega = nn.Parameter(torch.tensor(0.5))
        self.final_l2norm = final_l2norm

    def forward(self, palm_feat, vein_feat):
        # Z' and S' (L2 Normalization)
        palm_norm = F.normalize(palm_feat, p=2, dim=1)
        vein_norm = F.normalize(vein_feat, p=2, dim=1)
        
        # Ensure weight is between 0 and 1
        w = torch.clamp(self.omega, 0.0, 1.0)
        
        # Weighted Concatenation: L = [ωZ' , (1 - ω)S']
        # Note: Vein is Z, Palm is S
        fused = torch.cat([w * vein_norm, (1 - w) * palm_norm], dim=1)
...
def train_phase2(cnn_palm, cnn_vein, config, writer, feat_dim, local_dim):
    ...
    fusion_model = Stage2FusionMCF(in_dim=feat_dim, final_l2norm=True).to(config.device)
    ...
    for epoch in range(config.p2_epochs):
        ...
        if avg_val_acc > best_acc:
            ...
            torch.save({
                ...
                'classifier': classifier.state_dict()
            }, os.path.join(config.save_dir, 'stage2_MCF_best.pth'))
