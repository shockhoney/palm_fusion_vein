...
class TinyMobileFaceNet(nn.Module):
    ...
    def forward(self, x, return_spatial=False, return_all_spatial=False):
        x = self.conv1(x)
        x = self.conv2_dw(x)
        x = self.conv_23(x)
        x3 = self.conv_3(x)
...
class MobileFaceNet(nn.Module):
    ...
    def forward(self, x, return_spatial=False, return_all_spatial=False):
        ...
        if return_all_spatial:
            return [feat3, feat4, feat5, feat_map]
