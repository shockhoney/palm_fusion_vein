...
class Stage2FusionDWS(nn.Module):
    def __init__(self, in_dim=256):
        super(Stage2FusionDWS, self).__init__()
        # Small MLP Attention to compute dynamic mixing weights based on feature quality
    ...
    def forward(self, palm_feat, vein_feat):
        # Concat features to judge relative quality dynamically
        concat_feat = torch.cat([palm_feat, vein_feat], dim=1)
        
        # Output [B, 2] dynamic mixing weights
        weights = self.attention(concat_feat)
        w_palm = weights[:, 0].unsqueeze(1)
        w_vein = weights[:, 1].unsqueeze(1)
        
        # Apply dynamic weighted feature sum
        fused_feat = w_palm * palm_feat + w_vein * vein_feat
        return fused_feat
from utils.datasets_txt import TxtImageDataset, PairTxtDataset
...
def train_phase2(cnn_palm, cnn_vein, config, writer, feat_dim, local_dim):
    ...
    fusion_model = Stage2FusionDWS(in_dim=feat_dim).to(config.device)
    ...
    for epoch in range(config.p2_epochs):
        ...
        if avg_val_acc > best_acc:
            ...
            torch.save({
                ...
                'classifier': classifier.state_dict()
            }, os.path.join(config.save_dir, 'stage2_DWS_best.pth'))
